...
N=45;    % floor(45*(600/T)^0.3);(n,1);  %
n=45*50;  %总模拟的block数，如果想要画图清晰，n控制在2000左右
...
for i=1:n
  ...
  % [next_Difficulty,hardbit,next_target,targbit ]= getNextDiff_old( Dseri(i-N:i-1),STseri(i-N:i-1),T,N);
    [next_Difficulty,hardbit,next_target,targbit ]= getNextDiff_ZCASH( Dseri(i-N:i-1),STseri(i-N:i-1),T,N);
...
title(['Attack at D=',num2str(AttackIn),', Stop when D=',num2str(AttackOut),',Multiplier=',num2str(HRAttackerMulti)])
