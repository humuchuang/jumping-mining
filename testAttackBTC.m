...
HRAttackerMulti=3;       %攻击者算力倍数
HRAttacker=HRAttackerMulti*HRworker;
AttackIn=0.95;  %攻击者进入倍数
AttackOut=1.45; %攻击者退出倍数

N=2016;    % floor(45*(600/T)^0.3);(n,1);  %
n=2016*20;  %总模拟的block数，如果想要画图清晰，n控制在2000左右
...
for i=1:n
    ...
    if ifattack
        if i>N
            if (mod(i,2016)==0) && Attackposition==0
                Attackposition=1;
                HRnow=HRAttacker+HRworker;
            elseif (mod(i,2016)==0) && Attackposition==1
                Attackposition=0;
                HRnow=HRworker;
            end
        end
  ...
  % [next_Difficulty,hardbit,next_target,targbit ]= getNextDiff_old( Dseri(i-N:i-1),STseri(i-N:i-1),T,N);
    ...
    [next_Difficulty,hardbit,next_target,targbit ]= getNextDiff_BTC( Dseri(i-N:i-1),STseri(i-N:i-1),T,i);
...
title(['Attack cycle per 2016 block,Multiplier=',num2str(HRAttackerMulti)])
%%
